 ...
 *
 * Compiled to assets/build/frontend.js by wp-scripts.
 * Enqueued on public pages via the frontend enqueue action.
 *
 * Responsibilities:
 ...
 *
 * No React – vanilla ES2020, works on all WordPress-supported browsers.
 *
...
// Localised strings injected via wp_localize_script( 'apprco-frontend', 'apprcoFrontend', [...] ).
const i18n = ( window.apprcoFrontend && window.apprcoFrontend.i18n ) || {
	expired: 'Expired',
	oneDay:  '1 day left',
	nDays:   '%d days left',
};
	...
	document.querySelectorAll( '.appcon-vacancy-search' ).forEach( ( form ) => {
		...
		form.addEventListener( 'submit', ( e ) => {
			...
			const archiveHref = form.dataset.archiveUrl
				|| ( window.apprcoFrontend && window.apprcoFrontend.archiveUrl )
